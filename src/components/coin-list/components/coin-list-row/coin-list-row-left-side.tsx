...
export function CoinListRowLeftSide({coin}: { coin: Coin }){
   return (
      <div className="flex items-center gap-3 min-w-0">
         <img src={coin.image ?? coin.thumb} loading="lazy" alt={coin.id} width={28} height={28} className="rounded-full shrink-0" />
         <div className="flex flex-col min-w-0 text-neutral-900 dark:text-neutral-100">
            <Link to={'/coin/' + coin.id} className="font-semibold truncate cursor-pointer">
               {coin.name}
            </Link>
