...
export function calcPNL() {
  ...
  const positions = selectCoinId ? coinToolsData[selectCoinId]?.positions ?? [] : []; 
  const currentPrice = selectCoin ? selectCoin.current_price : 0; 
  const totalInvested = positions.reduce((sum: number, p) => sum + p.qty * p.price, 0);
  const currentValue = positions.reduce((sum: number, p) => sum + p.qty * currentPrice, 0);
